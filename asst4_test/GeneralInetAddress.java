...
public class GeneralInetAddress {
	...
	/**
	 * Returns an InetAddress representing the address of the localhost. If no
	 * other address can be found, the loopback will be returned.
	 * 
	...
	public static InetAddress getLocalHost() throws UnknownHostException {
		...
		for (int i = 0; i < addrs.length; i++) {
			if (addrs[i].isLoopbackAddress())
				continue;
			if (Pattern.matches("\\d+\\.\\d+\\.\\d+\\.\\d+", addrs[i].getHostAddress()))
				return addrs[i];
		}
		return localHost;
	}
