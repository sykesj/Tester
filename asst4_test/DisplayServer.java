...
public class DisplayServer extends JPanel implements KeyListener {
	...
	protected Color[] my_colors = new Color[] { Color.black, Color.blue, Color.cyan, Color.green, Color.magenta,
			Color.orange, Color.pink, Color.red, Color.yellow, Color.darkGray };
	...
	public class MessageListener extends Thread {
		...
		public MessageListener(Socket client, DisplayServer display) {
			...
			try {
				...
				my_client = new BufferedReader(new InputStreamReader(client.getInputStream()));
			} catch (IOException e) {
				...
				while (true) {
					...
					if (tok.equals("clear")) {
						...
						synchronized (my_display) {
							...
							} // end for (int i = 0; i < my_display.numVehicles;
							// i++)
	...
	public void keyPressed(KeyEvent e) {
		//TODO by students: FILL IN KEY ACTIONS HERE!
		// handle arrows by "keycode"
		switch (e.getKeyCode()) {
		case KeyEvent.VK_UP:
			// handle up
			break;
	...
	protected synchronized void drawVehicles(Graphics g) {
		...
		for (int j = 0; j < numVehicles; j++) {
			...
			for (int i = 0; i < 9; i++) {
				// We scale the x and y by 5, since the bounds on X and Y are
				// 100x100
